...
```{r}
#read rnf6080 into rain_df data frame
...
```{r}
#store number of rows in to a variable named n_rows
n_rows <- nrow(rain_df) 
#store number of columns in to a variable named n_cols
n_cols <- ncol(rain_df)   
...
```{r}
# Sum columns 4 through 27 (which is hour 0 to hour 23) and assign it to a new column called 'daily_rain_fall'
...
```{r}
#replace negative hourly rainfall with 0 (We will not use NA because NA values can't be added)
...
sum(x)
```
We would expect the following outcomes for numerical data: the maximum value being 12, the sorted output being 5, 7, 12, and the sum being 24. However, since 5, 12, and 7 are treated as character data types, the results differ. In lexicographical order, the maximum value is 7, as it's the highest when compared as strings. Sorting the values lexicographically yields 12, 5, and 7, and calculating the sum is not feasible with character data types.
...
y[2] + y[3]
```
The vector y contains mixed data types, and in R, when a vector holds different types of data, it is coerced to the most flexible type that can accommodate all values. In this case, that type is character.

As a result, y[2] and y[3] are treated as characters, which leads to an error when attempting to perform arithmetic operations on them.
...
z[1,2] + z[1,3]
```
We created a data frame with one row and three columns. The value in the first row, first column is the character "5", while the values in the first row, 
second column, and first row, third column are the numbers 7 and 12, respectively. When the second line of code adds the values from the first row, second column (7) and the first row, third column (12), both are numeric and can be added together, resulting in 19.

3. (3 pts, equally weighted). 

a.) What is the point of reproducible code? 
Reproducible code allows anyone, including original author, to product the same
code, regardless of environment and time. It allows validation and collaboration
, ensures that the code can trusted and built upon on.
